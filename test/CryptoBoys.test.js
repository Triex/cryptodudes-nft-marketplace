...
contract("Crypto Dudes", async (accounts) => {
  let cryptoDudes, result, cryptoDudeCount;

  before(async () => {
    cryptoDudes = await CryptoDudes.deployed();
  });

  describe("Deployment", async () => {
    it("contract has an address", async () => {
      const address = await cryptoDudes.address;
    ...
    it("has a name", async () => {
      const name = await cryptoDudes.collectionName();
      assert.equal(name, "Crypto Dudes Collection");
    });

    it("has a symbol", async () => {
      const symbol = await cryptoDudes.collectionNameSymbol();
  ...
  describe("application features", async () => {
    it("allows users to mint ERC721 token", async () => {
      cryptoDudeCount = await cryptoDudes.cryptoDudeCounter();
      assert.equal(cryptoDudeCount.toNumber(), 0);

      let tokenExists;
      tokenExists = await cryptoDudes.getTokenExists(1, { from: accounts[0] });
      assert.equal(tokenExists, false);

      let tokenURIExists;
      tokenURIExists = await cryptoDudes.tokenURIExists(
      ...
      let tokenNameExists;
      tokenNameExists = await cryptoDudes.tokenNameExists("myCBNFT", {
        from: accounts[0],
      });
      ...
      for (let i = 0; i < colorsArray1.length; i++) {
        colorExists = await cryptoDudes.colorExists(colorsArray1[i], {
          from: accounts[0],
        });
        assert.equal(colorExists, false);
      }

      result = await cryptoDudes.mintCryptoDude(
        "myCBNFT",
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPHRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2",
      ...
      cryptoDudeCount = await cryptoDudes.cryptoDudeCounter();
      assert.equal(cryptoDudeCount.toNumber(), 1);
      ...
      await cryptoDudes.mintCryptoDude(
        "myCBNFT2",
    ...
    it("returns address of the token's owner", async () => {
      const tokenOwner = await cryptoDudes.getTokenOwner(2);
      assert.equal(tokenOwner, accounts[1]);
    });

    // returns tokenURI of the token
    it("returns metadata of a token", async () => {
      const tokenMetaData = await cryptoDudes.getTokenMetaData(2);
      assert.equal(
        tokenMetaData,
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPQRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2"
      );
    ...
    it("returns total number of tokens minted so far", async () => {
      const totalNumberOfTokensMinted = await cryptoDudes.getNumberOfTokensMinted();
      assert.equal(totalNumberOfTokensMinted.toNumber(), 4);
    });

    it("returns total number of tokens owned by an address", async () => {
      const totalNumberOfTokensOwnedByAnAddress = await cryptoDudes.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
    ...
    it("allows users to buy token for specified ethers", async () => {
      const oldTokenOwner = await cryptoDudes.getTokenOwner(1);
      ...
      oldTotalNumberOfTokensOwnedBySeller = await cryptoDudes.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
      ...
      result = await cryptoDudes.buyToken(1, {
        from: accounts[2],
        value: web3.utils.toWei("1", "Ether"),
      });

      const newTokenOwner = await cryptoDudes.getTokenOwner(1);
      ...
      newTotalNumberOfTokensOwnedBySeller = await cryptoDudes.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
      ...
      assert.equal(cryptoDude.numberOfTransfers.toNumber(), 1);
      ...
      assert.equal(cryptoDude.currentOwner, accounts[2]);

      await cryptoDudes.buyToken(2, {
        from: 0x0000000000000000000000000000000000000000,
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;

      await cryptoDudes.buyToken(56, {
        from: accounts[4],
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;

      await cryptoDudes.buyToken(3, {
        from: accounts[0],
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;
    });
    ...
    it("allows users to change token price", async () => {
      let cryptoDudePrice;
      cryptoDudePrice = await cryptoDudes.allCryptoDudes(1, {
        from: accounts[0],
      });
      assert.equal(web3.utils.fromWei(cryptoDudePrice.price, "ether"), 1);

      result = await cryptoDudes.changeTokenPrice(
        1,
      ...
      assert.equal(web3.utils.fromWei(cryptoDudePrice.price, "ether"), 2);

      await cryptoDudes.changeTokenPrice(1, web3.utils.toWei("3", "Ether"), {
        from: 0x0000000000000000000000000000000000000000,
      }).should.be.rejected;

      await cryptoDudes.changeTokenPrice(82, web3.utils.toWei("3", "Ether"), {
        from: accounts[2],
      }).should.be.rejected;
    ...
    it("allows users to toggle between setting the token for sale or not for sale", async () => {
      ...
      await cryptoDudes.toggleForSale(94, { from: accounts[2] }).should.be
        .rejected;

      await cryptoDudes.toggleForSale(1, { from: accounts[8] }).should.be
        .rejected;
    });
  });
});
